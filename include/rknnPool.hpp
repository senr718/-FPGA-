...
rknn_lite::rknn_lite(char *model_name, int n)
{
    ...
    if (model_data == nullptr) // 新增：模型加载失败校验
    {
    ...
    // 适配RK3568：仅支持NPU CORE_0（忽略n的输入，强制使用CORE_0）
    rknn_core_mask core_mask = RKNN_NPU_CORE_0;
    printf("RK3568 use NPU CORE_0 (ignore input n=%d)\n", n); // 提示用户核心已固定
    // if (n == 0)
...
{
    cv::Mat img;
    // 获取图像宽高 ori_img yuanshituxiang
    int img_width = ori_img.cols;
    int img_height = ori_img.rows;
    cv::cvtColor(ori_img, img, cv::COLOR_BGR2RGB);//brg to rgb
    ...
    const float nms_threshold      = NMS_THRESH; // 非极大值抑制 (NMS) 的阈值
    const float box_conf_threshold = BOX_THRESH;// 目标检测框的置信度阈值
    float scale_w = (float) width / img_width;// 宽度方向的缩放比例
    float scale_h = (float) height / img_height;// 高度方向的缩放比例
